...
@onready var bpm_spinner: SpinBox = %BpmSpinner
...
func _ready():
	...
	bpm_spinner.value = stream.bpm
	bpm_spinner.value_changed.connect(func(val): stream.bpm = val)
...
func _on_compiled(success: bool, errors: Array):
	if success:
		status_label.text = "Compiled — %d samples loaded" % stream.get_loaded_samples().size()
